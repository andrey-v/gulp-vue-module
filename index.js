...
module.exports = function (options) {
    var defaults = {
        ...
        templateReplaceRegexp: 'template(?:\[\t| ])*?,', // vue component template replace tag
        loadCSSMethod:      'require("loadCSS")',    // define the load css method for require
        preDefDeps:         'require,exports,module,loadCSS'
    };

    var settings              = Object.assign({}, defaults, options),
        debug                 = settings.debug,
        indent                = settings.indent,
        templateReplaceRegexp = settings.templateReplaceRegexp,
        preDefDeps            = settings.preDefDeps;
    ...
    return through.obj(function (file, encoding, callback) {
        ...
        if (file.isBuffer()) {
            ...
            fragment.childNodes.forEach(function (node) {
                ...
                if (componentTags.indexOf(type) >= 0) {
                    ...
                    if (type === 'script') {
                        ...
                        script.split('\n').forEach(function (line) {
                            if (line.trim() != '') {
                                line = line.replace(new RegExp(templateReplaceRegexp, 'gi'), "template: '" + contents.template + "',");
            ...
            if (settings.amd && moduleDeps) {
                ...
                preDefDeps.split(/\s*,\s*/).forEach(function (dep) {
                    deps.push('"' + dep + '"');
                });
